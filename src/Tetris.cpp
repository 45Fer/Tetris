...
void Tetris::jugar() {
    ...
    while (ventana.estaAbierta() && !gameOver) {
        ...
        while (ventana.obtenerEvento(e)) {
            ...
            if (a == Controles::Bajar) { piezaActual->y++; if (tablero.colision(*piezaActual)) { piezaActual->y--; tablero.fijarPieza(*piezaActual); tablero.limpiarLineas(); nuevaPieza(); } }
